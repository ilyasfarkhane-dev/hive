...
export async function POST(request: NextRequest) {
  try {
    console.log('=== DEBUG: Simple Relationship Test ===');
    ...
    // Step 1: Create a simple record in icesc_project_suggestions
    console.log('=== STEP 1: Creating Record ===');
    const createData = {
      session: sessionId,
      module_name: 'icesc_project_suggestions',
      name_value_list: [
        { name: 'name', value: 'Simple Relationship Test' },
        { name: 'description', value: 'Testing relationship establishment' },
    ...
    // Step 2: Try to set subservice relationship
    ...
    // Step 3: Try to set contact relationship
    ...
    return NextResponse.json({
      ...
      message: 'Simple relationship test completed'
    });
    
  } catch (error) {
    console.error('Simple relationship test error:', error);
    return NextResponse.json({
      success: false,
