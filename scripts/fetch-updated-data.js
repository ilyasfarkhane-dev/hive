...
function processToServiceSubservicesData(subservices) {
  ...
  subservices.forEach((entry, index) => {
    const obj = {};
    ...
    if (serviceId) {
      ...
      unlinkedCount++;
      if (unlinkedCount <= 5) { // Only show first 5 warnings
        console.log(`⚠️ Subservice ${obj.id} (${obj.name}) has no service relationship`);
      }
    }
  });
  
  if (unlinkedCount > 5) {
    console.log(`⚠️ ... and ${unlinkedCount - 5} more subservices without service relationships`);
  }
  
  console.log(`✅ Processed ${processedCount} subservices into ${Object.keys(serviceSubservicesData).length} services`);
  console.log(`⚠️ ${unlinkedCount} subservices had no service relationships`);
...
async function main() {
  try {
    console.log('🚀 Fetching UPDATED data from CRM...');
    console.log('=====================================');
    ...
    if (Object.keys(serviceSubservicesData).length === 0) {
      console.log('❌ No services with subservices found. This might indicate a data structure issue.');
      console.log('💡 Try running the debug script to see the actual data structure.');
      return;
    }
    ...
    console.log('=====================================');
    console.log('✅ Updated data fetched and saved successfully!');
    ...
    const jsonPath = path.join(dataDir, 'updated-data-raw.json');
    ...
    console.log('4. Your original data has been restored and is still available');
    process.exit(1);
  }
}
